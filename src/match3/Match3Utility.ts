 ...
 * @param types List of types available to fill up slots
...
export function match3CreateGrid(rows = 6, columns = 6, types: Match3Type[]) {
  ...
  for (let r = 0; r < rows; r++) {
    for (let c = 0; c < columns; c++) {
      ...
      // If the new type match previous types, randomize it again, excluding rejected type
...
function matchPreviousTypes(grid: Match3Grid, position: Match3Position, type: Match3Type) {
  ...
  // Return if either horizontal or vertical positions are forming a match
 ...
 * ]
 * @param grid The grid to be analyzed
...
export function match3ApplyGravity(grid: Match3Grid) {
  ...
  for (let r = rows - 1; r >= 0; r--) {
    for (let c = 0; c < columns; c++) {
      ...
      // Retrieve the type of the position below
 ...
 * @param grid The grid to be changed
 * @param types List of types available to randomize
